...
| 09 | Dein Ort | **globe.gl · Kiosk** | Museums-Kiosk: 3 Fragen (Alter → Partei → Wohnort per Ort/PLZ, Offline-Geocoding via `plz.json`) → Datenpunkt fliegt auf einen **schwarz-weißen Globus** (Wireframe + Grid), Kamera taucht nach Deutschland ab, Landung exakt am eigenen Ort. Danach freies Erkunden: weiße **Besucher-Heatmap** (localStorage + BTW-2025-Seed), Hover-Tooltip pro Bundesland (Top-Partei je Altersgruppe), Scroll-Zoom/Drag, **60 s Idle → Reset**. Hooks: `?skipintro&plz=&age=&party=`, `?pov=lat,lng,alt`, `?fast`, `?idle=5`. |
